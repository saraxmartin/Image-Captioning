...
for model_function, model_name in zip(name_models, names):
    model = CaptioningModel(model_function, model_name, EMBEDDING_DIM, HIDDEN_DIM, VOCAB_SIZE, ATTENTION_DIM, method=2)
    model = model.to(DEVICE)
    CRITERION = selected_config["CRITERION"](ignore_index=2)  # Loss function
    OPTIMIZER = selected_config["OPTIMIZER"](model.parameters(), lr=selected_config["LEARNING_RATE"], weight_decay=0.0001)
    SCHEDULER = selected_config["SCHEDULER"](OPTIMIZER)
